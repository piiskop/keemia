...
## Konstandid ja ühikud

Konstandid ja ühikud on esitatud joonisel \@ref(fig:constants-units) leheküljel `r render_pageref(reference = "fig:constants-units")`.

```{r label = "constants-units", echo=FALSE, fig.cap = "Konstandid ja ühikud."}
include_external_graphics("rmd/baasreeglistik/konstandid-ühikud.svg")
...
```{=tex}
\begin{align}
I = \sum{\frac{\vec{p_i}}{\vec{v_i}} \times r_i^2} = \sum{\frac{\vec{p}}{\vec{\omega \times r}} \times r^2} \leavevmode\\ = \sum{\frac{\vec{\frac{\vec{L}}{\vec{r}}}}{\vec{\omega \times r}} \times r^2} = \frac{\sum{\vec{L}}}{\vec{\omega}}.
...
\begin{align}
\vec{V_1} \times (\vec{V_2} \times \vec{V_3})\leavevmode\leavevmode\\
...
```{=tex}
\begin{align}
\vec{T} = \vec{r} \times \vec{m \cdot \vec{a}} = m \cdot \vec{r} \times \vec{a} = \leavevmode\\
m \cdot \vec{r} \times \vec{\vec{r} \times \vec{\alpha}}\leavevmode\\
= m \cdot ((\vec{r} \cdot \vec{r}) \cdot \vec{\alpha} - (\vec{\alpha} \cdot \vec{r}) \cdot \vec{r}) =\leavevmode\\
m \times r^2 \times \vec{\alpha}\leavevmode\\
= I \cdot \vec{\alpha}.
...
A-tüüpi mõõtemääramatuse arvutamise valem usaldusmäära $1 - α$ korral on \@ref(eq:measurement-uncertainty-of-type-A) leheküljel `r render_pageref(reference="eq:measurement-uncertainty-of-type-A")`. Selles valemis:
...
B1-tüüpi mõõtemääramatus on esitatud valemina \@ref(eq:measurement-uncertainty-of-type-B1) leheküljel `r render_pageref(reference="eq:measurement-uncertainty-of-type-B1")`. Selles valemis
...
```{=tex}
\begin{equation}
u_\mathrm{B2} = (1 - \alpha) \cdot \frac{l}{2} \cdot n.
(\#eq:measurement-uncertainty-of-type-B2)
\end{equation}

```

Kui kõiki tüüpi mõõtemääramatused on välja arvutatud, tuleb mõõtemääramatused liita, nagu liidetakse vektoreid (avaldis \@ref(eq:measurement-uncertainty) leheküljel `r render_pageref(reference = "eq:measurement-uncertainty")`).
...
Lõpuks tuleb mõõdetud väärtus väljendada kujul, mis on esitatud

### Suhteline mõõtemääramatus

Suhteline mõõtemääramatus arvutatakse absoluutse mõõtemääramatuse ja mõõdetud keskväärtuse suhtena (valem \@ref(eq:relative-uncertainty) leheküljel `r render_pageref(reference = "eq:relative-uncertainty")`)[@uncertaintypdf, lk 22, 23; @mtmised, lk 16].
...
ruler_relative <- ruler_smallest_unit / ruler_x
ruler_data_frame <- data.frame(ruler_x_actual)

ruler_lower <- ruler_x_actual - ruler_smallest_unit_actual
ruler_upper <- ruler_x_actual + ruler_smallest_unit_actual

```

Minu kasutada oleva *deli* joonlaua kohta puuduvad tehaseandmed, mistõttu lähtun B-tüüpi mõõtemääramatuse arvutamises joonlaua väikseimast ühikust, milleks on skaalat $\frac{\num{1}}{\num{400}}$ kasutades \qty{`r as.character(ruler_smallest_unit)`}{\m} (joonis \@ref(fig:measuring-with-ruler-with-scales) leheküljel `r render_pageref(reference = "fig:measuring-with-ruler-with-scales")`[@noauthor_undated-ar]). Kuna kaardi mõõtkava on $\frac{\num{1}}{{4000}}$, siis on tegelikkuses lühim selle joonlauaga ja skaalaga mõõdetav vahemaa \qty{`r as.character(ruler_smallest_unit_actual)`}{\m}.
...
See joonlaud ei ole digitaalne, mistõttu jääb B-tüüpi mõõtemääramatuseks väikseim ühik.

Mõõdetud vahemaa on väljendatuna täielikult \qty{`r as.character(ruler_x_actual)`(`r as.character(ruler_smallest_unit_actual)`)}{\m}.
...
\begin{align}
R = \frac{\qty{`r as.character(ruler_smallest_unit)`}{\m}}{\qty{`r as.character(ruler_x)`}{\m}} =  \num{`r as.character(signif(x = ruler_relative, digits = 3))`}
(\#eq:ruler-relational-uncertainty)
\end{align}

Mõõdetud suurus esitatuna suhtelise mõõtemääramatusega on \qty{`r as.character(ruler_x)`}{\m}\pm\num{`r as.character(signif(x = ruler_relative, digits = 3))`}.
...
alpha <- 1 - .05 / 2
caliper_number_student = quantile(studentTDistribution, alpha)
student_7 <- quantile(StudentsT(df = 6), alpha)

caliper_uncertainty_A <- caliper_sd_mean * caliper_number_student

caliper_smallest_unit <- .01e-3
caliper_b <- caliper_smallest_unit / 2
#caliper_b_last <- caliper_b * 10
...
Vees lahustustuvus on esitatud tabelites \@ref(tab:solubility) leheküljel `r render_pageref(reference="tab:solubility")` ja \@ref(tab:solubility-ammonium) leheküljel `r render_pageref(reference="tab:solubility-ammonium")`.

(ref:solubility) Kilogrammi ühendi lahustuvus vees kilogrammi vee kohta[@haynes_2014_crc, lk 4-43-4-114]. Kui skalaarne lahustuvus ei ole teada, siis on lahustuvus märgitud järgnevalt: väga lahustuv: \qty{1}{\kg\per\kg}, lahustuv: \qty{1e-1}{\kg\per\kg}, kergelt lahustuv: \qty{1e-2}{\kg\per\kg}.

```{r label="solubility", echo=FALSE}
Li <- c(
  4.5e-1, # acetate
  1.81, # bromide
  1.30e-2, # carbonate
...
solubility <- mutate_all(.tbl=solubility, ~cell_spec(format_SI(.x), color=spec_color(.x), font_size = spec_font_size(.x)))
print_table(table=solubility, caption="(ref:solubility)", additional_header=c("Anioon"=1, "Muldmetallid"=5, "Leelismuldmetallid"=4, "Siirdemetallid"=4, "Järgne"=1)) %>%
  landscape()

```

(ref:solubility-ammonium) Kilogrammi ühendi lahustuvus vees kilogrammi vee kohta[@haynes_2014_crc, lk 4-43-4-114]. Kui skalaarne lahustuvus ei ole teada, siis on lahustuvus märgitud järgnevalt: väga lahustuv: \qty{1}{\kg\per\kg}, lahustuv: \qty{1e-1}{\kg\per\kg}, kergelt lahustuv: \qty{1e-2}{\kg\per\kg}.

```{r label="solubility-ammonium", echo=FALSE}
NH4 <- c(
  1.48, # acetate
  7.83e-1, # bromide
  1, # carbonate
...
solubility_ammonium <- data.frame(
  NH4
)

colnames(solubility_ammonium) <- c(
  "\\ce{[NH4]+}"
)
rownames(solubility_ammonium) <- c(
  "\\ce{[CH3CO2]-}",
  "\\ce{Br-}",
  "\\ce{[CO3]^2-}",
  "\\ce{[ClO3]-}",
...
solubility_ammonium <- mutate_all(.tbl=solubility_ammonium, ~cell_spec(format_SI(.x), color=spec_color(.x), font_size = spec_font_size(.x)))
print_table(table=solubility_ammonium, caption="(ref:solubility-ammonium)", additional_header=c("Anioon"=1, "Ammoonium"=1)) %>%
  landscape()
